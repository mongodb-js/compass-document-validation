...
describe('<RuleBuilder />', function() {
  ...
  beforeEach(function() {
    ...
    this.RuleBuilder = require('../../lib/components/rule-builder');
  });
  afterEach(function() {
  ...
  context('when collection is not writable', function() {
    ...
    it('disables the ADD RULE button', function() {
      const state = this.component.find('.btn.btn-xs.btn-primary');
      expect(state).to.be.disabled();
    });

    it('shows tooltip indicating why button is disabled', function() {
